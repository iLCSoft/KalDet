                ...
                : TVKalDetector(300) // SJA:FIXME initial size, 300 looks reasonable for ILD, though this would be better stored as a const somewhere
{
  ...
  for (int layer=0; layer<_nLayers; ++layer) {
	...
	if(ladder==0){   // bacause overlap section of ladder0 is further outer than the last ladder.
	  ...
	  Add(new ILDPlanarMeasLayer(air, silicon, sen_front_face_centre, normal, _bZ, sen_front_sorting_policy, pos_xi_nonoverlap_width - ladder_xi_min, length, (pos_xi_nonoverlap_width + ladder_xi_min)/2, active, layerID )) ;
	  ...
	  double overlap_front_sorting_policy    = sensitive_distance + (3* nLadders+0) * eps;
	  double overlap_back_sorting_policy     = sensitive_distance + (3* nLadders+1) * eps;
	  double overlap_sup_back_sorting_policy = sensitive_distance + (3* nLadders+2) * eps;

	  // air - sensitive boundary
	  Add(new ILDPlanarMeasLayer(air, silicon, sen_front_face_centre, normal, _bZ, overlap_front_sorting_policy, overlap_region_width, length, overlap_region_offset, active, layerID )) ;
	  streamlog_out(DEBUG3) << "ILDVXDKalDetector add surface with layerID = "
				<< layerID
				<< std::endl ;
	...
	double sup_forward_sorting_policy = ladder_distance + (3 * ladder+0) * eps ;
	double sup_back_sorting_policy =    ladder_distance + (3 * ladder+1) * eps ;
	double sen_back_sorting_policy =    ladder_distance + (3 * ladder+2) * eps ;
	...
	Add(new ILDPlanarMeasLayer(carbon, silicon, sup_back_face_centre, normal, _bZ, sup_back_sorting_policy, width, length, offset, active, layerID )) ; 
	streamlog_out(DEBUG3) << "ILDVXDKalDetector add surface with layerID = "
			      << layerID
			      << std::endl ;
